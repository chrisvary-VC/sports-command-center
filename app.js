...
root.style.setProperty('--ticker-speed', `${config.tickerSpeedSeconds || 42}s`);
...
const statusScore = (status='') => {
  ...
  if (s.includes('FEATURED') || s.includes('RACE WEEK')) return config.priorityWeights?.upcoming || 20;
  return 10;
};

const heroSlides = data.heroSlides
  .filter((s)=>enabledSports.includes(s.league))
  .map((s)=>({...s, priority:statusScore(s.status)+(containsFavorite(s)?(config.priorityWeights?.favorite||35):0)}))
...
function tickerHTML(items){ const filtered=items.filter(([tag])=>enabledSports.includes(tag)); const source=filtered.length?filtered:items; return [...source,...source].map(([tag,text])=>`<span class="ticker-item"><b>${tag}</b>${text}</span>`).join(''); }
$('breakingTicker').innerHTML=tickerHTML(data.breaking); $('upcomingTicker').innerHTML=tickerHTML(data.upcoming);

const visibleLeagues=data.leagues.filter((l)=>enabledSports.includes(l.name));
...
function renderHero(){
  ...
  const progress=$('heroProgress'); progress.style.animation='none'; requestAnimationFrame(()=>progress.style.animation=`heroProgress ${config.heroRotationMs||11000}ms linear infinite`);
}
renderHero();
setInterval(()=>{ const hero=$('hero'); hero.classList.add('switching'); setTimeout(()=>{heroIndex=(heroIndex+1)%heroSlides.length; renderHero(); hero.classList.remove('switching');},350); },config.heroRotationMs||11000);
...
renderStory(); setInterval(()=>{storyIndex=(storyIndex+1)%stories.length;renderStory();},config.storyRotationMs||8500);
